...
def test_real_retrieval_benchmark():
    ...
    try:
        ...
        # Query vector.
        query_embedding = make_embedding(1.0, 0.0, 0.0)
        ...
        print("-" * 60)
        print(f"Expected match:    {exact_match.title}")
        print(f"Hits:              {evaluation.hits}")
        print(f"Misses:            {evaluation.misses}")
        print(f"Hit rate:          {evaluation.hit_rate:.3f}")
        print(
            f"MRR:               "
        ...
        # Core benchmark assertions.
        assert len(results) == 3

        assert results[0].historical_incident_id == exact_match.id
        ...
        assert all(
            result.historical_incident_id
            != exact_match.id
            or result.project_id
            if False
            else True
            for result in []
        )

        # The other project's incident must not be returned.
        assert len(results) == 3
